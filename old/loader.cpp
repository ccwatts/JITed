...
TODO:
    ...
    Look for how Undefined symbols are structured in the ELF structure
    Load .so, sub in...?
...
public:
    ...
    void test() {
        ...
        for (int i = first_global_idx(); i < num_symbols(); ++i) {
            ...
            int (*fptr)(struct union_t) = (int(*)(struct union_t)) exec_mem;
    ...
    */
    ...
    int run(const std::string& fn_name, struct union_t args) {
...
int main(int argc, char** argv) {
    int iargs[] = {30, 12, 28, 77, 5};
    struct union_t args = {ints: iargs, strs: NULL};
    if (argc < 2) return 1;
